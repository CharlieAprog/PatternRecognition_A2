...
for labeled_ratio in labeled_ratios:
    per_approach = {}
    for approach in ['semisupervised', 'supervised', 'justlabelpropagation']:
        ...
        per_approach[approach] = (mean(ratio_corrects), np.std(ratio_corrects), np.std(ratio_corrects) / np.sqrt(10))
